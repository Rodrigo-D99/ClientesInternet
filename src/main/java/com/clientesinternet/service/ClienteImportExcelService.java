    ...
    public int importarClientes(MultipartFile file) throws Exception {
        ...
        try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                String mesesAdeudadosStr = getCell(row, 13);
                int mesesAdeudadosInicial = (mesesAdeudadosStr != null) ? (int) Double.parseDouble(mesesAdeudadosStr) : 0;

                // Construcción de la entidad Cliente incluyendo la deuda inicial importada
